#!/usr/bin/env python3
"""
esp32c5_suite_ext2.py
11-test hardware suite for ESP32-C5 (extends suite_ext with loopback tests).

New tests vs suite_ext:
  AEL_INTR  GPIO interrupt   GPIO2(drive) → GPIO3(input, reuse PCNT jumper)
  AEL_UART  UART1 loopback   GPIO4(TX) ↔ GPIO5(RX)
  AEL_ADC   ADC loopback     GPIO6(drive) → GPIO1(ADC1_CH0)
  AEL_SPI   SPI loopback     GPIO7(MOSI) ↔ GPIO9(MISO), CLK=GPIO0, CS=GPIO10
...
FIRMWARE_DIR = os.path.join(PROJECT_ROOT, "firmware", "targets", "esp32c5_suite_ext2")
BUILD_DIR    = os.path.join(PROJECT_ROOT, "artifacts",  "build_esp32c5_suite_ext2")
...
EXPECTED = [
    ...
    "AEL_INTR", "AEL_UART", "AEL_ADC", "AEL_SPI",
]
DONE_SENTINEL = "AEL_SUITE_EXT2 DONE"
...
def build_firmware() -> None:
    print("[BUILD] esp32c5_suite_ext2 …")
...
def read_uart_until_done(port: str, timeout_s: float = 45.0) -> list[str]:
    import serial as _s
    lines: list[str] = []
    try:
...
def main() -> int:
    ...
    def _uart_worker():
        # WiFi dual-band ~12s + BLE 3s + boot 2s + 4 new tests + margin = 45s
        uart_lines.extend(read_uart_until_done(console_port, timeout_s=45.0))
        uart_done.set()

    threading.Thread(target=_uart_worker, daemon=True).start()
    uart_done.wait(timeout=50.0)
