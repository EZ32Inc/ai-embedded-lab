#!/usr/bin/env python3
"""
esp32c5_suite_ext.py
Extended hardware suite for ESP32-C5:
Temperature, NVS, Wi-Fi dual-band scan, BLE passive scan,
Light sleep, PWM (LEDC self-test), PCNT pulse count.

Wiring:
  GPIO2 <-> GPIO3  jumper (PCNT loopback)
  No LA.

Ports:
  Flash   — Native USB Serial/JTAG  serial 3C:DC:75:84:A6:54
  Console — CH341 UART0 bridge      serial 5AAF278818
"""
...
FIRMWARE_DIR = os.path.join(PROJECT_ROOT, "firmware", "targets", "esp32c5_suite_ext")
BUILD_DIR    = os.path.join(PROJECT_ROOT, "artifacts",  "build_esp32c5_suite_ext")

FLASH_SERIAL   = "3C:DC:75:84:A6:54"   # ESP32-C5 native USB Serial/JTAG
CONSOLE_SERIAL = "5AAF278818"           # CH341 UART0 bridge (GPIO11 TX / GPIO12 RX)
...
def read_uart_until_done(port: str, timeout_s: float = 35.0) -> list[str]:
    """Read UART0 until AEL_SUITE_EXT DONE or timeout."""
    import serial as _s
    lines: list[str] = []
    try:
...
def main() -> int:
    ...
    def _uart_worker():
        # boot 2s + WiFi dual-band ~12s + BLE 3s + sleep 1s + margin → 35s
        uart_lines.extend(read_uart_until_done(console_port, timeout_s=35.0))
        uart_done.set()

    threading.Thread(target=_uart_worker, daemon=True).start()
    uart_done.wait(timeout=40.0)
