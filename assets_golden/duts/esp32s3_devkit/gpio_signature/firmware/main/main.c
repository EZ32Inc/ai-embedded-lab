...
static const char *TAG = "ESP32S3 DUT";
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "Hello wrold from ESP32S3 DUT by AEL");
    printf(
    ...
    while (1) {
        // Keep a periodic ready line so post-flash UART capture can reliably match.
        printf("AEL_DUT_READY\n");
