...
def build_nightly_report_payload(date_str: str, summary: dict) -> Dict:
    ...
    return {
        ...
        "capability_taxonomy_versions": str(review.get("capability_taxonomy_versions", "unavailable")),
        "status_health_schema_versions": str(review.get("status_health_schema_versions", "unavailable")),
        "doctor_check_schema_versions": str(review.get("doctor_check_schema_versions", "unavailable")),
        "capability_taxonomy_enforced": str(review.get("capability_taxonomy_enforced", "unavailable")),
        "status_taxonomy_enforced": str(review.get("status_taxonomy_enforced", "unavailable")),
        "doctor_checks_enforced": str(review.get("doctor_checks_enforced", "unavailable")),
...
def _render_nightly_report_markdown(payload: Dict) -> str:
    ...
    lines.extend(
        [
            ...
            f"- instrument_families: {review.get('instrument_families', 'unavailable')}",
            f"- instrument_health: {review.get('instrument_health', 'unavailable')}",
            f"- failure_boundaries: {review.get('failure_boundaries', 'unavailable')}",
            f"- recovery_hints: {review.get('recovery_hints', 'unavailable')}",
            f"- capability_taxonomy_versions: {payload.get('capability_taxonomy_versions', review.get('capability_taxonomy_versions', 'unavailable'))}",
            f"- status_health_schema_versions: {payload.get('status_health_schema_versions', review.get('status_health_schema_versions', 'unavailable'))}",
            f"- doctor_check_schema_versions: {payload.get('doctor_check_schema_versions', review.get('doctor_check_schema_versions', 'unavailable'))}",
            f"- capability_taxonomy_enforced: {payload.get('capability_taxonomy_enforced', review.get('capability_taxonomy_enforced', 'unavailable'))}",
            f"- status_taxonomy_enforced: {payload.get('status_taxonomy_enforced', review.get('status_taxonomy_enforced', 'unavailable'))}",
            f"- doctor_checks_enforced: {payload.get('doctor_checks_enforced', review.get('doctor_checks_enforced', 'unavailable'))}",
            "```text",
