...
def run_reset(instrument: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    ...
    commands += ["monitor reset run", "disconnect"]
    output = gdb_batch(endpoint, commands, timeout_s=15, gdb_cmd=gdb_cmd)
    lines = [line for line in output.splitlines() if line.strip()]
    return {
        "status": "success",
        "action": "reset",
        "data": {"method": "monitor reset run"},
        "logs": lines,
    }
