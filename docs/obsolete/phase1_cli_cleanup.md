...
- Added [`ael/config_resolver.py`](../ael/config_resolver.py) as the central policy layer for:
  - control-instrument config resolution
  - notify control-instrument policy
  - board config defaults
  - doctor required tool list
- Refactored [`ael/__main__.py`](../ael/__main__.py) to call resolver helpers instead of embedding board/control-instrument/tool-specific policy.
- Added regression check script: [`tools/check_resolver_defaults.py`](../tools/check_resolver_defaults.py).

## Why
This removes board/control-instrument/tool-specific policy from CLI orchestration code and keeps Core cleaner.  
...
- `ael pack` still applies board-based notify-control-instrument policy as before.
