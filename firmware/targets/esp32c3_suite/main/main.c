/*
 * esp32c3_suite  —  10-test hardware suite
 *
 * Self-contained loopback tests for ESP32-C3 single-USB (native) devkit.
 * No external instrument required.
 * Console output via USB Serial/JTAG CDC → /dev/ttyACM0.
 *
 * Wiring (4 jumpers required):
 *   GPIO4  ↔  GPIO5    GPIO interrupt (drive + input)
 *   GPIO6  ↔  GPIO7    UART1 TX ↔ RX loopback
 *   GPIO2  →  GPIO1    GPIO output → ADC1_CH1 input
 *   GPIO10 ↔  GPIO3    SPI2 MOSI ↔ MISO loopback (CLK=GPIO21, CS=GPIO20)
 *
 * PWM: GPIO8 (no jumper needed)
 *
 * No PCNT — ESP32-C3 has no PCNT peripheral (SOC_PCNT_SUPPORTED not defined).
 * No I2C test — ESP32-C3 has only one I2C controller; insufficient free
 * GPIO pairs for master+slave loopback alongside other tests.
 *
 * Output tags:
 ...
 *   - Console: CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=y (USB CDC, not UART0)
 *   - GPIO20/21 (UART0) free because console is on USB
 *   - No PMP_IDRAM_SPLIT constraint (different RISC-V variant from C5)
 *   - BLE torn down after test (task priority hygiene)
 */
...
{
    const uint32_t WVAL = 0xAEC30001U;
    nvs_handle_t h;
    esp_err_t e = nvs_open("ael_c3", NVS_READWRITE, &h);
...
void app_main(void)
{
    ...
    printf("AEL_SUITE_C3 DONE passed=%d failed=%d\n", s_passed, s_failed);  /* expect 10/10 */
    fflush(stdout);
}
