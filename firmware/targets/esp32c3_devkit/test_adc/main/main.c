/* test_adc — Stage 2: ADC1 loopback. GPIO8 drives -> GPIO1 (ADC1_CH1).
 * GPIO8 is safe_output and not LP-IO/ADC-capable, so it can drive HIGH
 * reliably while ADC1 is active.  GPIO2 (ADC1_CH2/LP-IO) cannot output
 * HIGH once the ADC1 unit is initialised on ESP32-C3.
 */
#include <stdio.h>
...
#define ADC_DRIVE      GPIO_NUM_8   /* safe_output, not LP-IO */
